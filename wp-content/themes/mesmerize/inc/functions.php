...
function mesmerize_do_enqueue_assets()
{
    ...
    $theme        = wp_get_theme();
    $ver          = $theme->get('Version');
    $isChildTheme = ($theme->get('Template') !== false);
...
function mesmerize_title()
{
    ...
    $value = apply_filters('mesmerize_header_title', mesmerize_wp_kses_post($title['title']));
...
{
    add_theme_page('mesmerize_theme_page', __('Mesmerize Info', 'mesmerize'), 'activate_plugins', 'mesmerize-welcome', 'mesmerize_load_theme_partial');
}
