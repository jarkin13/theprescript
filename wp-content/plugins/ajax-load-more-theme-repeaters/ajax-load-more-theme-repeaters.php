...
define('ALM_THEME_REPEATERS_VERSION', '1.0.9');
define('ALM_THEME_REPEATERS_RELEASE', 'November 16, 2017');
   	...
   	function alm_get_theme_repeater($theme_repeater, $alm_found_posts, $alm_page, $alm_item, $alm_current){
         ...
         if(!isset($options['_alm_theme_repeaters_dir'])) 
	   	...
	   	if($theme_repeater != 'null'){
		   	...
		   	if(is_child_theme()){		      
					$file = get_stylesheet_directory().'/'.$options['_alm_theme_repeaters_dir'].'/'.$theme_repeater;
		      }else{		  
		   		$file = get_template_directory().'/'.$options['_alm_theme_repeaters_dir'].'/'.$theme_repeater;  	         
		      }
		   	
		   	if(!file_exists($file)) //confirm file exists        			
